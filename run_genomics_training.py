...
from quixer.setup_genomics import setup_genomics_dataset, get_genomics_train_evaluate
...
class SimpleClassifier(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        bottleneck_dim = max(1, min(hidden_dim, bottleneck_dim))
...
def main():
    ...
    parser.add_argument('--quixer_qubits', type=int, default=6,
                        help='Number of qubits used by the Quixer circuit')
    parser.add_argument('--quixer_layers', type=int, default=3,
                        help='Degree of the QSVT polynomial (circuit depth)')
    parser.add_argument('--quixer_ansatz_layers', type=int, default=4,
                        help='Number of layers in the token ansatz circuit')
    parser.add_argument('--quixer_dropout', type=float, default=0.10,
                        help='Dropout rate applied before PQC angle projection')
    parser.add_argument('--quixer_lr', type=float, default=0.002,
                        help='Learning rate for Quixer training')
    parser.add_argument('--quixer_weight_decay', type=float, default=0.0001,
                        help='Weight decay for Quixer training')
    parser.add_argument('--quixer_epochs', type=int, default=30,
                        help='Number of epochs for Quixer training')
    ...
    for model_name in args.models:
        ...
        if model_name.lower() == 'quixer':
            train_eval = get_genomics_train_evaluate(device)
            quixer_hparams = {
                'model': 'Quixer',
                'batch_size': args.batch_size,
                'epochs': args.quixer_epochs,
                'lr': args.quixer_lr,
                'wd': args.quixer_weight_decay,
                'dropout': args.quixer_dropout,
                'qubits': args.quixer_qubits,
                'layers': args.quixer_layers,
                'ansatz_layers': args.quixer_ansatz_layers,
            }
            train_eval(quixer_hparams)
            # Metrics are printed inside train_eval; skip JSON aggregation for now
            continue

        # Initialize classical baselines locally
        ...
        # Train and evaluate classical models via local loop
        results = train_model(
    ...
    if all_results:
        ...
        # Print summary comparison for recorded models
        print(f"\n{'='*70}")
